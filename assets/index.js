...
class BasketPopup extends Popup {
    constructor() {
        super({
            ...
            afterShow: () => {
                $('.popup-content tr a', this.$el)[0].focus();
            },
        });
        $('.popup-content .delete-button', this.$el).each(deleteButton => {
            new LinkButton(deleteButton);
        });
        const $popupContent = $('.popup-content', this.$el);
...
class TextareaPopup extends Popup {
    constructor() {
        ...
        const $popupContent = $('.popup-content', this.$el);
        const $textarea = $('textarea', $popupContent);
